...
\begin{table*}[!htpb]
    ...
    \midrule
        fl dur          & \tabincell{l}{Flow duration} & pkt len max     & \tabincell{l}{Maximum length of a flow} \\
        tot fw pk       & \tabincell{l}{Total packets in the forward direction} & pkt len avg     & \tabincell{l}{Mean length of a flow} \\
        tot bw pk       & \tabincell{l}{Total packets in the backward direction} & pkt len std     & \tabincell{l}{Standard deviation length of a flow} \\
        tot l fw pkt    & \tabincell{l}{Total size of packet in forward direction} & pkt len va      & \tabincell{l}{Minimum inter-arrival time of packet} \\
        fw pkt l max    & \tabincell{l}{Maximum size of packet in forward direction} & fin cnt         & \tabincell{l}{Number of packets with FIN} \\
        fw pkt l min    & \tabincell{l}{Minimum size of packet in forward direction} & syn cnt         & \tabincell{l}{Number of packets with SYN} \\
        fw pkt l avg    & \tabincell{l}{Average size of packet in forward direction} & rst cnt         & \tabincell{l}{Number of packets with RST} \\
        fw pkt l std    & \tabincell{l}{Standard deviation size of packet in forward \\ direction} & pst cnt         & \tabincell{l}{Number of packets with PUSH} \\
        Bw pkt l max    & \tabincell{l}{Maximum size of packet in backward direction} & ack cnt         & \tabincell{l}{Number of packets with ACK} \\
        Bw pkt l min    & \tabincell{l}{Minimum size of packet in backward direction} & urg cnt         & \tabincell{l}{Number of packets with URG} \\
        Bw pkt l avg    & \tabincell{l}{Mean size of packet in backward direction} & cwe cnt         & \tabincell{l}{Number of packets with CWE} \\
        Bw pkt l std    & \tabincell{l}{Standard deviation size of packet in backward \\ direction} & ece cnt         & \tabincell{l}{Number of packets with ECE} \\
        fl byt s        & \tabincell{l}{flow byte rate that is number of packets \\ transferred per second} & down up ratio   & \tabincell{l}{Download and upload ratio} \\
        fl pkt s        & \tabincell{l}{flow packets rate that is number of packets \\ transferred per second} &  pkt size avg    & \tabincell{l}{Average size of packet} \\
        fl iat avg      & \tabincell{l}{Average time between two flows} & fw seg avg      & \tabincell{l}{Average size observed in the forward direction} \\
        fl iat std      & \tabincell{l}{Standard deviation time two flows} & bw seg avg      & \tabincell{l}{Average size observed in the backward direction} \\
        fl iat max      & \tabincell{l}{Maximum time between two flows} & fw byt blk avg  & \tabincell{l}{Average number of bytes bulk rate in \\ the forward direction} \\
        fl iat min      & \tabincell{l}{Minimum time between two flows} & fw pkt blk avg  & \tabincell{l}{Average number of packets bulk rate in \\ the forward direction} \\
        fw iat tot      & \tabincell{l}{Total time between two packets sent in the \\ forward direction} & fw blk rate avg & \tabincell{l}{Average number of bulk rate in the \\ forward direction} \\
        fw iat avg      & \tabincell{l}{Mean time between two packets sent in the \\ forward direction} & bw byt blk avg  & \tabincell{l}{Average number of bytes bulk rate in \\ the backward direction} \\
        fw iat std      & \tabincell{l}{Standard deviation time between two packets \\ sent in the forward direction} & bw pkt blk avg  & \tabincell{l}{Average number of packets bulk rate in \\ the backward direction} \\
        fw iat max      & \tabincell{l}{Maximum time between two packets sent in \\ the forward direction} & bw blk rate avg & \tabincell{l}{Average number of bulk rate in \\ the backward direction} \\ 
        fw iat min      & \tabincell{l}{Minimum time between two packets sent in \\ the forward direction} & subfl fw pk     & \tabincell{l}{The average number of packets in \\ a sub flow in the forward direction} \\
        bw iat tot      & \tabincell{l}{Total time between two packets sent in \\ the backward direction} & subfl fw byt    & \tabincell{l}{The average number of bytes in \\ a sub flow in the forward direction} \\
        bw iat avg      & \tabincell{l}{Mean time between two packets sent in \\ the backward direction} & subfl bw pkt    & \tabincell{l}{The average number of packets in \\ a sub flow in the backward direction} \\ 
        bw iat std      & \tabincell{l}{Standard deviation time between two packets \\ sent in the backward direction} & subfl bw byt    & \tabincell{l}{The average number of bytes in \\a sub flow in the backward direction} \\
        bw iat max      & \tabincell{l}{Maximum time between two packets sent in \\ the backward direction} & fw win byt      & \tabincell{l}{Number of bytes sent in initial window \\ in the forward direction} \\
        bw iat min      & \tabincell{l}{Minimum time between two packets sent in \\ the backward direction} &  bw win byt      & \tabincell{l}{Number of bytes sent in initial window in \\ the backward direction} \\
        fw psh flag     & \tabincell{l}{Number of times the PSH flag was set in \\ packets travelling in the forward direction \\ (0 for UDP)} & Fw act pkt      & \tabincell{l}{Number of packets with at least 1 byte of TCP data \\ payload in the forward direction} \\
        bw psh flag     & \tabincell{l}{Number of times the PSH flag was set in \\ packets travelling in the backward direction \\ (0 for UDP)} & fw seg min      & \tabincell{l}{Minimum segment size observed in the \\ forward direction} \\
        fw urg flag     & \tabincell{l}{Number of times the URG flag was set in \\ packets travelling in the forward direction \\ (0 for UDP)} & atv avg         & \tabincell{l}{Mean time a flow was active before becoming idle} \\
        bw urg flag     & \tabincell{l}{Number of times the URG flag was set in \\ packets travelling in the backward direction \\ (0 for UDP)} & atv std         & \tabincell{l}{Standard deviation time a flow was active before \\ becoming idle} \\
        fw hdr len      & \tabincell{l}{Total bytes used for headers in the forward \\ direction} & atv max         & \tabincell{l}{Maximum time a flow was active before becoming idle} \\
        bw hdr len      & \tabincell{l}{Total bytes used for headers in the forward \\ direction} & atv min         & \tabincell{l}{Minimum time a flow was active before becoming idle} \\
        fw pkt s        & \tabincell{l}{Number of forward packets per second} & idl avg         & \tabincell{l}{Mean time a flow was idle before becoming active} \\
        bw pkt s        & \tabincell{l}{Number of backward packets per second} & idl std         & \tabincell{l}{Standard deviation time a flow was idle before \\becoming active} \\
        pkt len min     & \tabincell{l}{Minimum length of a flow} & idl max         & \tabincell{l}{Maximum time a flow was idle before becoming \\ active} \\
        & & idl min         & \tabincell{l}{Minimum time a flow was idle before becoming \\ active} \\
    \bottomrule
    \end{tabular}
\end{table*}
...
\begin{enumerate}
    ...
    \item Some miscellaneous. In our result, the ACK Flag count is selected in our feature subset. In fact the number of ACKs accounts for a large proportion of the TCP flow. % TODO: analysis detailed representation of ACK Flag cnt or delete it.
\end{enumerate} 
...
\begin{table}[!htpb]
    ...
    Fwd Seg Size Min    & \tabincell{l}{Minimum segment size observed in \\ the forward direction} \\
    Init Fwd Win Byts   & \tabincell{l}{Number of bytes sent in initial window in the \\ forward direction} \\
    Flow Pkts/s         & \tabincell{l}{flow packets rate that is number of packets \\ transferred per second} \\
    ACK Flag Cnt        & Number of packets with ACK \\
    Flow IAT Mean       & Average time between two flows \\ 
    Bwd IAT Max         & \tabincell{l}{Maximum time between two packets sent in \\ the backward direction} \\
    ...
    Fwd IAT Tot         & \tabincell{l}{Total time between two packets sent in the \\ forward direction} \\
    \bottomrule
    \end{tabular}
\end{table}
