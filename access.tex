...
\begin{abstract}
    ...
    Experimental results show that \change{}{on CIC-IDS-2018 data set, }the proposed scheme 
    can decrease the training time \change{}{from ... to ...(number)} and increase the classification metrics\change{}{accuracy, ..., from...to...} comparing to training the classifier with full data set. 
    Under the condition of same number of selected features, the proposed method can also significantly increase the classification metrics including accuracy, precision, recall and F1-score \change{}{as comparing to Seleck-K-Best method with chi-square method}.
\end{abstract}
...
All experiments are running on a quad-core Intel PC with 2.90GHz CPU and 16 GB of memory. Our algorithms are implemented in Python 3.7 and the chi-square method is implemented in a Python library scikit-learn\cite{sklearn}. \change{The conducted experiments are about the following aspects:}{We investigate the impact of distance thresholds and feature spaces on the number of clusters and classification results.}

\begin{itemize}
    \item \change{The effect of different distance thresholds on the number of clusters.}{} We compare the number of clusters \change{in}{corresponding to} different threshold $\delta$, then explain how to choose best threshold to improve the classification metrics best.
    \item \change{The effect of different size feature spaces on classification results.}{} \change{We compare the training time and classification metrics of the features selected by this algorithm and all the features in the complete data set on the decision tree classifier respectively.}{We compared the training time and classification index of this method and the naive method, respectively. In the naive method, the data set is not processed, and all features are used for anomaly detection.}
    \item \change{The effect of feature space of the same size but different elements on classification results.}{} We compare the training time and classification index of the features selected by this algorithm and the features selected by the chi-square method on the decision tree classifier when the number of features is the same.
\end{itemize}
